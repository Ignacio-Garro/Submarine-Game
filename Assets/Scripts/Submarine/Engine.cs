...
public class Engine : NetworkBehaviour
{
    ...
    public float GetMotorUsablePercentage(float percentage)
    {

        float absPercentage = Mathf.Abs(percentage);
        tryToUsePercent = absPercentage;
        return isWorking ? percentage : 0f;

    }

    public void UseMotorPercentage(float percentageUsage)
    {
        pressureLevel.Value += (tryToUsePercent * percentageUsage - pressureLevel.Value) * Time.deltaTime / timeToRegulatePressure;
    }
